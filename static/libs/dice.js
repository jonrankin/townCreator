...
(function() {
    ...
    function createDie(type, labelColor, dieColor, houseNumber) {
        labelColor = labelColor || defaultLabelColor;
        dieColor = dieColor || defaultDieColor;
	houseNumber = houseNumber || defaultHouseNumber;
	...
	var arrowHelper = new THREE.ArrowHelper( dir, origin, length, hex, length*0.2, (length*0.2)*0.5 );
	...
	*/
	...
	lineGeo.vertices.push(
		new THREE.Vector3(-(this.cw),0,0),
		new THREE.Vector3(this.cw,0,0)
	);
	lineGeo.vertices.push(
                new THREE.Vector3(0,-(this.ch),0),
                new THREE.Vector3(0,this.ch,0)
        );
    ...
    this.dieBox.prototype.createDie = function(type, pos, velocity, angle, axis, labelColor, color, houseNumber) {
        var die = createDie(type, labelColor, color, houseNumber);
        die.dieType = type;
    ...
    this.dieBox.prototype.check = function() {
		...
		values.diceType[dice.dieType].push(matindex);
            }
            if (this.callback) {
	...
	console.log(this.renderer.getClearColor());
    };
    ...
    this.dieBox.prototype.searchByHouseNumber = function(houseNumber) {
           var diceProp;
	   this.dice.forEach(function(dice) {
		if (dice.userData.houseNumber == houseNumber){ diceProp = dice.position; }
	   });
	...
	//console.log(this.aspect);
 	...
 	var intersects = (new THREE.Raycaster(this.camera.position, 
                                              (new THREE.Vector3((((ev.clientX - this.cw) / this.aspect)- $t.id('canvas').getBoundingClientRect().left) ,
                                                                 (((ev.clientY - this.ch) / this.aspect) - $t.id('canvas').getBoundingClientRect().top )*-1, this.w/9))
                                              .sub(this.camera.position).normalize())).intersectObjects(this.dice);

	/*var mousePosition = new THREE.Vector3((((ev.clientX - this.cw) / this.aspect)- $t.id('info_div').clientWidth) ,
                                                                 ((ev.clientY - this.ch) / this.aspect)*-1, this.w/9);*/
	//console.log(mousePosition);

	if (intersects.length) {
	    /*alert("found");
	     console.log(new THREE.Raycaster(this.camera.position,
	    ...
	    return Object.assign(intersects["0"].object.userData, intersects["0"].object.position);
        }
    };
